...
这是相当不幸的。在这种情况下，我们希望接受生命周期至少与 `'world` 一样长的任何类型。
让我们尝试使用生命周期进行子类型化。
...
所以，问题在于，一旦 `&'static str` 被放在 `&mut` 引用后面，我们就不能假设它仍然可以被降级为 `&'world str` 来满足 `T` 的要求。
这意味着，即使 `'static` 是 `'world` 的子类型，`&mut &'static str` 也 **不能** 是 `&mut &'world str` 的子类型。
...
此外，我们注意到不能将 `&mut &'a T` 视为 `&mut &'b T` 的子类型，因此我们可以说 `&mut T` 在 `T` 上是 *不变的*
...
所以当函数被调用时，调用者只期望一个生命周期至少为 `'a` 的 `&str` 的值，至于这个值的生命周期是不是比 `'a` 更长，并不重要。
